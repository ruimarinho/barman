...
class TestGetSnapshotInterface(object):
    ...
    def test_from_config_aws(self, mock_boto3):
        ...
        mock_config = mock.Mock(
            ...
            aws_snapshot_cool_off_period=2,
            aws_snapshot_expiration_date=datetime.datetime(2024,1,1),
        )
        ...
        assert snapshot_interface.cool_off_period == 2
        assert snapshot_interface.expiration_date == datetime.datetime(2024,1,1)
...
class TestAwsCloudSnapshotInterface(object):
    ...
    def _get_mock_lock_snapshot_resp(self, snapshot_id, lock_mode, cool_off_period, lock_duration):
        ...
        return {
            ...
            "CoolOffPeriodExpiresOn": (lock_created_on + datetime.timedelta(hours=cool_off_period)).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + 'Z',
            ...
            "LockDurationStartTime": (lock_created_on + datetime.timedelta(hours=cool_off_period)).strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3] + 'Z'
        }

    def _get_mock_describe_locked_snapshots_resp(self, snapshot_id, lock_mode, cool_off_period, lock_duration, owner_id="123456789012", next_token=None):
    ...
    def _get_mock_lock_snapshot_resp(self, snapshot_id, lock_mode, cool_off_period, lock_duration, expiration_date):
        """Helper which returns a mock lock_snapshot response."""
        lock_created_on = datetime.datetime(2024, 1, 1)
    ...
    def test_take_snapshot_backup_with_lock(self, number_of_disks, mock_ec2_client):
        ...
        snapshot_interface = AwsCloudSnapshotInterface(
            ...
            lock_duration=1,
            cool_off_period=2,
            expiration_date=datetime.datetime(2024,1,1),
        )
